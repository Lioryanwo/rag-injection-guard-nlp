...
# ── Shared helpers (inlined to avoid circular import) ────────────────────────
...
_GENERIC_PHRASES = [
    ...
    "evidence suggests", "notably", "interestingly",
    "however, the exact", "the precise role remains",
]

_ANSWER_LIKE_TEMPLATES = [
    ...
    r"(while|although)\s+the\s+(exact|specific|precise)\s+(answer|detail|information)",
    r"(some|a\s+number\s+of)\s+(researchers|scholars|experts)\s+(argue|suggest|claim)",
    r"(this|the)\s+(passage|text|document|chunk)\s+(discusses|addresses|covers)",
]
_ANSWER_RE = [re.compile(p, re.IGNORECASE) for p in _ANSWER_LIKE_TEMPLATES]
...
def _generic_phrase_score(text: str) -> float:
    lc = text.lower()
    hits = sum(1 for p in _GENERIC_PHRASES if p in lc)
    return min(1.0, hits * 0.15)


def _answer_template_score(text: str) -> float:
    hits = sum(1 for p in _ANSWER_RE if p.search(text))
    return min(1.0, hits * 0.25)


def _query_stuffing_score(text: str, query_words: Set[str]) -> float:
    if not query_words:
        return 0.0
    words = _cwords(text)
    if not words:
    ...
    return min(1.0, max(0.0, (ratio - 0.20) / 0.20))


def _near_duplicate_score(text: str, seen_shingles: Dict, k: int = 5) -> float:
    words = _cwords(text)
    if len(words) < k:
        return 0.0
    ...
    for prev in seen_shingles:
        ...
        if union == 0:
            continue
        jaccard = len(shingles & prev) / union
        max_overlap = max(max_overlap, jaccard)
    seen_shingles[shingles] = seen_shingles.get(shingles, 0) + 1
    return min(1.0, max_overlap)


def _compute_suspicion(chunk: Dict, query_words: Set[str], seen_shingles: Dict) -> Tuple[float, Dict]:
    text = chunk.get("text", "")
    if not text:
        return 0.0, {}
    ld = _lexical_diversity(text)
    gp = _generic_phrase_score(text)
    at = _answer_template_score(text)
    qs = _query_stuffing_score(text, query_words)
    nd = _near_duplicate_score(text, seen_shingles)
    score = min(1.0,
        0.20 * max(0.0, 0.65 - ld) +
        0.30 * gp +
        0.25 * at +
        0.15 * qs +
        0.10 * nd
    ...
    return score, {
        ...
        "generic_phrase":    round(gp, 4),
        "answer_template":   round(at, 4),
        "query_stuffing":    round(qs, 4),
        "near_duplicate":    round(nd, 4),
...
def filter_corpus(
    ...
    for q in query_texts:
        for w in re.findall(r"\b[a-zA-Z][a-zA-Z'\-]+\b", q.lower()):
            if w not in STOPWORDS and len(w) > 2:
                all_query_words.add(w)

    seen_shingles: Dict = {}
    kept, removed, scored = [], [], []

    for chunk in chunks:
        score, breakdown = _compute_suspicion(chunk, all_query_words, seen_shingles)
        entry = {**chunk, "suspicion_final": round(score, 4),
                 "suspicion_breakdown": breakdown, "filtered_out": score >= threshold}
        scored.append(entry)
        (removed if score >= threshold else kept).append(entry)

    return kept, removed, scored


def evaluate_filter(kept: List[Dict], removed: List[Dict], all_chunks: List[Dict]) -> Dict:
    ...
    return {
        ...
        "precision_of_removal": round(removed_spoof / max(1, len(removed)), 4),
    }
...
def build_filtered_index(
    ...
    texts = [
        f"passage: {c['text']}" if "e5" in model_name.lower() else c["text"]
        for c in kept_chunks
    ]
    print(f"  Embedding {len(texts)} kept chunks…")
    ...
    print(f"  Index: {len(kept_chunks)} chunks → {output_dir}")
