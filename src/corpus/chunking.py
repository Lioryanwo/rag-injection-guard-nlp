...
=============================================================================
Purpose:
The second script in the pipeline. It reads the full documents and splits them 
into smaller, overlapping text segments ("chunks"). This is necessary to allow 
the vector search engine to process text efficiently without losing information 
at the split boundaries. The script also prepends the document title to each 
chunk to maintain semantic context even for mid-document chunks.

Inputs:
- corpus_docs.jsonl (generated by the previous step).
- Parameters: --chunk-size (words per chunk), --overlap (overlapping words), 
  and --prepend-title.

Outputs (saved to data/processed/):
- corpus_chunks.jsonl: The chunked documents, ready for vector encoding.
=============================================================================
"""
...
# Read and write JSONL files, and split text into overlapping word-level chunks.
...
def main() -> None:
    ...
    logger.info(f"Settings: chunk_size={args.chunk_size}, overlap={args.overlap}, prepend_title={args.prepend_title}")
    ...
    logger.info(f"Chunking complete. Created {len(chunk_rows)} chunks.")
    logger.info(f"Saved chunks to: {args.output_path}")
